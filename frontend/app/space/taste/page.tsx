...
export default async function TastePage() {
  ...
  let perceptionAvailable = true;
  let { data, error } = await supabase.from("brew_logs").select("score,acidity,sweetness,bitterness,body,balance,aroma,perceived_flavor_notes,beans(flavor_notes,bean_profiles(predicted_acidity,predicted_sweetness,predicted_body))").eq("user_id", userId!);
  if (error && (error.code === "42703" || error.code === "PGRST204") && error.message?.includes("perceived_flavor_notes")) {
    perceptionAvailable = false;
    const fallback = await supabase.from("brew_logs").select("score,acidity,sweetness,bitterness,body,balance,aroma,beans(flavor_notes,bean_profiles(predicted_acidity,predicted_sweetness,predicted_body))").eq("user_id", userId!);
    data = fallback.data?.map((row) => ({ ...row, perceived_flavor_notes: null })) ?? null;
  ...
  const logs: TasteLog[] = (data ?? []).map((row) => {
    ...
    return { score: row.score, acidity: row.acidity, sweetness: row.sweetness, bitterness: row.bitterness, body: row.body, balance: row.balance, aroma: row.aroma, perceived_flavor_notes: "perceived_flavor_notes" in row ? row.perceived_flavor_notes : null, flavor_notes: bean?.flavor_notes, bean_profile: beanProfile };
  });
  ...
  const perceivedFamilies = calculateFlavorFamilies(logs, "perceived").slice(0, 5);
  ...
  return <section className="space-welcome taste-page"><p className="kicker"><span /> Understand yourself</p><h1>My Taste</h1>
    ...
    {automatic.contributingBrews > 0 && <section className="automatic-preference"><span>AUTOMATIC COFFEE PREFERENCE</span><div><h2>What the coffees you enjoy have in common.</h2><p>Based on reference profiles and your liking—not on sensory answers. Only dimensions available in the reference profile are shown.</p></div><dl>{[
      ["Acidity", automatic.dimensions.acidity], ["Natural sweetness", automatic.dimensions.sweetness], ["Body", automatic.dimensions.body],
    ].map(([label, value]) => <div key={String(label)}><dt>{label}</dt><dd>{value === null ? "—" : `${value} / 5`}</dd><progress max="5" value={Number(value ?? 0)} /></div>)}</dl><small>Based on {automatic.contributingBrews} liked {automatic.contributingBrews === 1 ? "brew" : "brews"} with a reference profile.</small></section>}
    {perceptionAvailable && <div className="flavor-families"><span>FLAVORS YOU EXPLICITLY TASTED</span>{perceivedFamilies.length ? <div>{perceivedFamilies.map((family) => <article key={family.family}><h3>{family.family}</h3><p>{family.brewCount} contributing {family.brewCount === 1 ? "brew" : "brews"}</p></article>)}</div> : <p>Add your own flavor words to a liked journal entry to build this evidence.</p>}</div>}
    <div className="flavor-families reference-flavor-families"><span>REFERENCE FLAVORS IN COFFEES YOU ENJOY</span>{families.length ? <div>{families.map((family) => <article key={family.family}><h3>{family.family}</h3><p>{family.brewCount} contributing {family.brewCount === 1 ? "brew" : "brews"}</p></article>)}</div> : <p>Add reference flavor notes to your Coffees and a liking score above five to see these families.</p>}<small>From saved product or roaster descriptions—not from your sensory answers.</small></div>
  </section>;
}
