...
export function LandingStory() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setBrewExpanded(active === 1), active === 1 && !reducedMotion ? 900 : 0);
    ...
    <div className="story-demo" aria-label="Example of how Beanmemo grows with each coffee">
      ...
      <div className="story-stage">
        ...
        <div className={`story-panel story-panel-${active}`} id={`story-panel-${active}`} role="tabpanel" aria-labelledby={`story-tab-${active}`}>
          {active === 0 && <div className="story-bean-card"><span>On your shelf · Example</span><div className="story-bean-heading"><b>Morning Espresso</b><p>Independent Roaster · Indonesia</p></div><div className="story-bean-tags"><small>Bold &amp; Roasty</small><small>Passion Fruit</small><small>Spicy</small></div><div className="story-bean-profile"><div className="story-profile-heading"><span>At a glance</span><b>Bold &amp; Roasty</b></div>{[["Roast", "5/5", "Dark", "100%"], ["Intensity", "5/5", "Intense", "100%"], ["Acidity", "1/5", "Very Low", "20%"]].map(([label, value, meaning, score]) => <div className="story-profile-row" key={label}><dt>{label}</dt><dd>{value}</dd><i><span style={{ width: score }} /></i><small>{meaning}</small></div>)}<p><b>Main flavors:</b> Passion Fruit · Spicy · Roasted</p></div></div>}
          {active === 1 && <div className="story-journal-card"><div className="story-journal-toolbar"><span>Group entries</span><b>By Bean</b></div><div className="story-journal-group"><header><span>Independent Roaster</span><b>Morning Espresso</b><p>2 entries · Best 8.5/10 · Latest Sep 10</p></header><button aria-expanded={brewExpanded} onClick={() => { setBrewExpanded((value) => !value); setPaused(true); }} type="button"><span>Sep 10</span><span><small>Method</small>Espresso Machine</span><span><small>Grind</small>5</span><b>8.5/10<small>Best so far</small></b><i>{brewExpanded ? "−" : "+"}</i></button>{brewExpanded && <div className="story-journal-detail"><dl><div><dt>Dose</dt><dd>18 g</dd></div><div><dt>Actual yield</dt><dd>36 g</dd></div><div><dt>Actual time</dt><dd>28 sec</dd></div><div><dt>Drink</dt><dd>Espresso</dd></div></dl><blockquote>Smooth and chocolatey, slightly bitter at the end.</blockquote><p><span>Observed</span><b>Balanced</b><b>Bitter finish</b></p><p><span>Next time</span>Try one step coarser</p></div>}</div><div className="story-journal-group story-journal-group-secondary"><header><span>Local Roaster</span><b>Guatemala Reserve</b><p>3 entries · Best 8/10 · Latest Sep 8</p></header></div><small className="story-equipment-line">My equipment · Italian-style espresso machine · External electric grinder</small></div>}
